...
func New(ctx context.Context, local bool, db pool.Pool, instanceConfig *config.InstanceConfig) (*Impl, error) {
	...
	gp, err := providers.New(ctx, instanceConfig.GitRepoConfig.Provider, instanceConfig.GitRepoConfig.URL, instanceConfig.GitRepoConfig.Branch, instanceConfig.GitRepoConfig.StartCommit, instanceConfig.GitRepoConfig.GitAuthType, instanceConfig.GitRepoConfig.Dir)
	if err != nil {
		return nil, skerr.Wrap(err)
	}
