...
describe('details-page-sk', () => {
  ...
  describe('url params', () => {
    it('correctly extracts the grouping and digest', async () => {
      ...
      expect(await detailsPageSkPO.digestDetailsSkPO.getLeftDigest()).to.equal(
        'Left: 99c58c7002073346ff55f446d47d6311'
      );
      expect(await detailsPageSkPO.digestDetailsSkPO.getRightDigest()).to.equal(
        'Right: 6246b773851984c726cb2e1cb13510c2'
      );

      // This link should not have a changelist ID or CRS.
      expect(
        await detailsPageSkPO.digestDetailsSkPO.getDiffPageLink()
      ).to.equal(
