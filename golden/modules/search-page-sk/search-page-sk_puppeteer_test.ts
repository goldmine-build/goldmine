...
describe('search-page-sk', () => {
  ...
  const goToPage = async (queryString = '') => {
    ...
    searchPageSkPO = new SearchPageSkPO(
      (await testBed.page.$('search-page-sk'))!
    );
  };

  beforeEach(async () => {
    eventPromiseFactory = await addEventListenersToPuppeteerPage(testBed.page, [
      'busy-end',
    ]);
  });
  ...
  it('reads search params from the URL', async () => {
    ...
    expect(await searchControlsSkPO.isIncludeUntriagedDigestsCheckboxChecked())
      .to.be.true;
    expect(await searchControlsSkPO.isIncludePositiveDigestsCheckboxChecked())
      .to.be.true;
    expect(await searchControlsSkPO.isIncludeNegativeDigestsCheckboxChecked())
      .to.be.true;
